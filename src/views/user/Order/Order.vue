<template>
  <div class="main-panel">
    <div class="content-wrapper">
      <order-detail-modal :order_id="order_id" v-if="show_detail" @close-modal="showModal()" @update-order="getData(order_status_id)"/>
      <div class="row">
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-8">
                            <select class="form-control" v-model="order_status_id">
                              ...
                              <option v-for="(item, index) in order_statuses" :value="item.id" :key="index">{{item.status}}</option>
                            </select>
                          </div>
                        </div>
                      </div>
                    ...
                    <tbody>
                      <tr v-for="(order, index) in orders" :key="index">
                        ...
                        <td>
                          <div>
                            <span class="d-flex align-items-center gap-1"
                              ><span class="fs-6 fw-bolder">{{ order.receiver }}</span>
                        ...
                        <td><label class="badge" 
                            :class="{'badge-info' : order.order_status_id == 1,
                                      ...
                                      'badge-light' : order.order_status_id > 5,}">{{ order.status.status }}</label></td>
                        <td class="action">
                          <label class="badge badge-info m-1" @click="showModal(order.id)" >Chi tiết</label>
                          <label class="badge badge-success m-1">Mua lại</label>
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
const orders = computed(() => store.state.myOrders ?? null);
...
const getData = (order_status_id) => store.dispatch("getMyOrders",{order_status_id: order_status_id,  token: store.state.token} );

watch( order_status_id, async() => await getData(order_status_id.value))

onBeforeMount( async () => {
  await getData(order_status_id.value);
});
